...
function sheetConnectionForSuite(suiteId, { readyOnly = false } = {}) {
  const targetSuiteId = String(suiteId || "").trim();
  if (!targetSuiteId) return null;
  return state.sheetConnections.find((connection) =>
    connection.suiteId === targetSuiteId &&
      ...
      <form method="dialog" class="pdf-export-scope-shell">
        ...
        <div class="pdf-export-scope-options" role="radiogroup" aria-label="${tr("出力対象", "Export scope")}">
          <label class="pdf-export-scope-option">
            ...
            <span>
              <strong>${tr("すべて", "All cases")}</strong>
              <small>${tr("実行対象 {count} 件を出力", "Export {count} runnable cases", { count: formatLocaleNumber(totalCount) })}</small>
            </span>
          </label>
          <label class="pdf-export-scope-option ${failedCount ? "" : "is-disabled"}">
            <input type="radio" name="pdf-scope" value="failed" ${failedCount ? "" : "disabled"}>
            <span>
              <strong>${tr("失敗のみ", "Failed only")}</strong>
              <small>${failedCount
                ? tr("不合格 {count} 件のみ出力", "Export {count} failed cases only", { count: formatLocaleNumber(failedCount) })
                : tr("不合格ケースはありません", "There are no failed cases")}</small>
            </span>
          </label>
        </div>
        <footer>
...
function buildQuickSearchCatalog() {
  ...
  const pages = [
    ...
    },
    {
      ...
      groupLabel: tr("ページ", "Pages"),
      title: "Google Sheets",
      subtitle: tr("接続状態と認証の診断", "Connection and authentication diagnostics"),
...
function renderSuites() {
  const cards = state.suites
    ...
    .join("");
  app.innerHTML = shell(`
    ${pageHead(tr("テストスイート", "Test suites"), tr("実業務プロンプトをまとめて実行し、品質とコストを継続評価します。", "Run real-world prompts together and continuously evaluate quality and cost."), `<button id="new-suite" class="button primary">${icon("plus", 16)}${tr("新しいスイート", "New suite")}</button>`)}
...
function suiteSheetConnectionDialog(suite, connection) {
  const connected = Boolean(connection);
  const statusCopy = connection?.status === "ready"
    ? tr("接続済み", "Connected")
    : connection
      ? tr("接続を確認してください", "Connection needs attention")
      : tr("未接続", "Not connected");
  return `<dialog id="suite-sheet-dialog" class="suite-sheet-dialog" aria-labelledby="suite-sheet-dialog-title" aria-describedby="suite-sheet-dialog-description">
    <form method="dialog" id="suite-sheet-form" class="suite-sheet-dialog-shell">
      ...
      <label>${tr("GoogleスプレッドシートURL / Spreadsheet ID", "Google spreadsheet URL / Spreadsheet ID")}
        <input name="spreadsheetUrl" required autocomplete="off" value="${esc(connection?.spreadsheetUrl || "")}" placeholder="https://docs.google.com/spreadsheets/d/.../edit">
      </label>
      <aside class="suite-sheet-safety">${icon("lock-keyhole", 17)}<div><strong>${tr("PrismTrail管理タブだけを更新します", "Only PrismTrail-managed tabs are updated")}</strong><p>${tr("このスプレッドシートは1つのテストスイート専用になります。既存のその他のタブは変更しません。", "This spreadsheet will be dedicated to one test suite. Other existing tabs are left unchanged.")}</p></div></aside>
...
function renderEditor() {
  ...
  const basicsPanel = `<section class="basic-panel">
            ...
            <section class="suite-sheet-card ${sheetReady ? "is-connected" : connectedSheet ? "needs-attention" : "is-empty"}">
              <span class="suite-sheet-card-icon">${icon("sheet", 19)}</span>
  ...
  const casesPanel = `
          ${suite.cases.length ? "" : `<section class="suite-start-panel">
            ...
            <div class="sheet-direct-row">
              ...
              <div><strong>${sheetReady ? esc(connectedSheet.title || "連携済みGoogle Sheets") : "Google Sheetsと連携"}</strong><small>${sheetReady ? "シート上でケースを編集し、アプリへ取り込めます。" : "連携すると、ここから編集用シートを直接開けます。"}</small></div>
              ${sheetReady
                ? `<button id="open-linked-sheet-inline" class="button sheet-link" type="button">${icon("sheet", 15)}${tr("同期してGシートを開く", "Sync and open Sheets")}${icon("external-link", 13)}</button>`
                : `<button class="button secondary" type="button" data-open-suite-sheet>${icon("link", 15)}${tr("Google Sheetsを連携", "Connect Google Sheets")}</button>`}
            </div>
          </section>`}
  ...
  app.innerHTML = shell(`
    <div class="editor-shell">
      ...
      ${suiteSheetConnectionDialog(suite, connectedSheet)}
    </div>`, "suites", "editor");
  refreshIcons();
  bindEditor();
  syncCaseNavScroll();
  if (state.suitePasteOpen) document.querySelector("#suite-paste-dialog")?.showModal();
  if (state.suiteSheetModalOpen) document.querySelector("#suite-sheet-dialog")?.showModal();
}
...
function clipPreviewText(value, max = 8_000) {
...
function reportCasePendingHtml(testCase, index, { active = null, isCancelling = false, interrupted = false } = {}) {
  const phaseLabel = {
    ...
    ? tr("実行が中断されました", "Run was interrupted")
    : isCancelling
      ? tr("中止待ち", "Waiting to stop")
      : tr("実行待ち", "Waiting"));
  return `<article class="report-case ${active ? "case-running" : interrupted ? "case-interrupted" : "case-pending"}">
    <header>
      <span class="${active ? "live-spinner" : "case-index"}">${active ? "" : String(index + 1).padStart(2, "0")}</span>
      <div><strong>${esc(testCase.title)}</strong><small>${active ? phaseLabel : phaseLabel}</small></div>
      <b>${active ? (isCancelling ? tr("中止中", "Cancelling") : tr("実行中", "Running")) : interrupted ? tr("中断", "Interrupted") : "—"}</b>
    </header>
...
function beginSuiteRunNavigation(suite, { caseIds = null } = {}) {
  ...
  const cases = (suite.cases || [])
    ...
    .filter((item) => requestedIds || String(item.status || "").toLowerCase() !== "draft");
...
function bindEditor() {
  ...
  const linkedDataButtonLabel = () =>
    `${icon("sheet", 15)}${tr("同期してGシートを開く", "Sync and open Sheets")}${icon("external-link", 13)}`;
  ...
  const openLinkedSheet = async () => {
    const connection = sheetConnectionForSuite(state.selectedSuite?.id, { readyOnly: true });
    if (!connection) {
      state.suiteSheetModalOpen = true;
  ...
  document.querySelectorAll("[data-open-suite-sheet]").forEach((button) => button.addEventListener("click", () => {
    state.suiteSheetModalOpen = true;
    document.querySelector("#suite-sheet-dialog")?.showModal();
  }));
  ...
  sheetDialog?.addEventListener("close", () => {
    ...
    if (location.hash === `#/suites/${state.selectedSuite?.id}/edit/sheets`) {
      history.replaceState(null, "", `#/suites/${state.selectedSuite.id}/edit`);
    }
  });
  document.querySelector("#suite-sheet-form")?.addEventListener("submit", async (event) => {
    ...
    const errorBox = form.querySelector("#suite-sheet-form-error");
    const input = form.elements.spreadsheetUrl;
    if (button.disabled) return;
    errorBox.hidden = true;
    errorBox.textContent = "";
    button.disabled = true;
    button.innerHTML = `${icon("loader-circle", 15)}${tr("接続を確認中…", "Checking connection…")}`;
    button.setAttribute("aria-busy", "true");
    refreshIcons();
    try {
      if (document.querySelector("#suite-name")) state.selectedSuite = collectSuite();
      const connection = await json("/api/sheets/connections", {
        ...
        body: JSON.stringify({
          ...
          sheetName: state.selectedSuite.name,
          spreadsheetUrl: input.value.trim()
        })
      });
      updateSheetConnection(connection);
      state.suiteSheetModalOpen = false;
      sheetDialog.close();
      notify(tr("このテストスイートのGoogle Sheets接続を保存しました。", "Saved the Google Sheets connection for this test suite."), "success");
      renderEditor();
    } catch (error) {
      errorBox.textContent = translateApiMessage(error.message);
      errorBox.hidden = false;
      button.disabled = false;
      button.innerHTML = `${icon("link", 15)}${sheetConnectionForSuite(state.selectedSuite?.id) ? tr("接続先を保存", "Save destination") : tr("接続して保存", "Connect and save")}`;
  ...
  document.querySelectorAll("[data-check-sheet]").forEach((button) => button.addEventListener("click", async () => {
    ...
    try {
      const connection = await json(`/api/sheets/connections/${button.dataset.checkSheet}/check`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ suiteId: button.dataset.sheetSuite })
      });
      updateSheetConnection(connection);
      if (connection.bootstrap?.suiteBootstrapped || connection.bootstrap?.reportBootstrapped || connection.bootstrap?.catalogsBootstrapped) {
        notify(
          tr(
            "接続を確認し、Suite専用の管理タブを同期しました。",
            "Connection verified and suite-scoped managed tabs were synced."
          ),
          "success"
        );
      } else {
...
function renderSettings() {
  ...
  app.innerHTML = shell(`
    ...
    <nav class="settings-tabs" role="tablist" aria-label="設定カテゴリ">
      ...
      <button class="settings-tab ${state.settingsTab === "sheets" ? "active" : ""}" type="button" role="tab" aria-selected="${state.settingsTab === "sheets"}" data-settings-tab="sheets">${icon("sheet", 15)}<span><strong>Google Sheets</strong><small>${tr("認証・接続診断", "Authentication · diagnostics")}</small></span></button>
...
function reportCaseNavItemHtml(entry, selectedCaseId) {
  const { testCase, item, index, active, interrupted = false } = entry;
  const caseId = testCase.id || testCase.caseId;
  const status = item?.status || (active ? "running" : interrupted ? "cancelled" : "pending");
  const score = item?.evaluation?.score;
  const grade = scoreGrade(score);
  const pendingLabel = active
    ? tr("実行中", "Running")
    : interrupted
      ? tr("中断", "Interrupted")
      : tr("待機", "Pending");
  return `<button type="button" class="report-case-nav-item ${status} ${caseId === selectedCaseId ? "selected" : ""}" data-report-case-id="${esc(caseId)}" aria-current="${caseId === selectedCaseId ? "true" : "false"}">
    ...
    <span class="case-nav-result"><b>${grade || "—"}</b><small>${item ? responseReceiptLabel(item.responseReceipt) : pendingLabel}</small></span>
  </button>`;
}
...
function renderReport(report, { evidenceByCaseId = null, selectedCaseId = null } = {}) {
  ...
  const allSuiteCases = report.suiteSnapshot?.cases || report.caseRuns || [];
  // ケース別評価は実行対象（下書き以外）のみ。過去 run で snapshot に draft が残っていても隠す。
  const suiteCases = allSuiteCases.filter(
    (testCase) => String(testCase.status || "").toLowerCase() !== "draft"
  );
  ...
  // 完了／失敗後に残った activeCases は「実行中」表示に使わない（クラッシュ後の見た目バグ防止）
  const liveActiveCases = isLive
  ...
  const caseEntries = suiteCases.map((testCase, index) => {
    const caseId = testCase.id || testCase.caseId;
    const item = completedCases.get(caseId) || null;
    const active = liveActiveCases.find((entry) => entry.caseId === caseId) || null;
    return {
      testCase,
      index,
      item,
      active,
      interrupted: Boolean(!item && !active && staleActiveIds.has(caseId))
    };
  });
  ...
  const selectedCaseDetail = selectedEntry?.item
    ...
    : selectedEntry
      ? reportCasePendingHtml(selectedEntry.testCase, selectedEntry.index, {
          ...
          isCancelling,
          interrupted: selectedEntry.interrupted
        })
      : empty(
  ...
  const runningCount = isLive
    ? report.summary?.running ?? (liveActiveCases.length || 0)
    : 0;
  const concurrency = report.summary?.concurrency || 30;
  const displayTotal = suiteCases.length || total;
  const displayCompleted = caseEntries.filter((entry) => entry.item).length;
  const displayProgress = displayTotal ? Math.round((displayCompleted / displayTotal) * 100) : 0;
  ...
  const reportSheetConnection = sheetConnectionForSuite(report.suiteId, { readyOnly: true });
  const sheetSettingsHref = report.suiteId
    ? `#/suites/${encodeURIComponent(report.suiteId)}/edit/sheets`
    : "#/suites";
  ...
  app.innerHTML = shell(`
    ...
    ${detailBody(`
      ...
      <div class="overall-score"><span>${isLive ? tr("実行進捗", "Run progress") : tr("総合スコア", "Overall score")}</span><strong>${isLive ? displayCompleted : overallScore ?? "—"}<small>/${isLive ? displayTotal : 100}</small></strong>${!isLive && businessConfigured > 0 ? `<em>${tr("システム 40% + ビジネス 60%", "System 40% + business 60%")}</em>` : !isLive ? `<em>${tr("ビジネス要件未評価のためシステムスコアを採用", "System score used because business requirements were not evaluated")}</em>` : ""}</div>
      ...
      ${isLive ? `<div class="live-progress"><span style="width:${displayProgress}%"></span></div>` : ""}
    </section>
  ...
  document.querySelector("#export-report-pdf")?.addEventListener("click", async () => {
    ...
    const failedCount = (report.caseRuns || []).filter((item) =>
      ["failed", "error"].includes(item.status)
    ).length;
    const totalCount = runnableCases.length || (report.caseRuns || []).length;
    let scope = "all";
...
function renderRunDetail(run) {
  ...
  document.querySelector("#export-context-report-pdf")?.addEventListener("click", async () => {
    ...
    const runnableCases = (suiteRun.suiteSnapshot?.cases || []).filter(
      (item) => String(item.status || "").toLowerCase() !== "draft"
    );
    const failedCount = (suiteRun.caseRuns || []).filter((item) =>
      ["failed", "error"].includes(item.status)
    ).length;
    const scope = await askPdfExportScope({
      totalCount: runnableCases.length || (suiteRun.caseRuns || []).length,
      failedCount
    });
    if (!scope) return;
    await withButtonBusy(button, tr("PDF生成中…", "Generating PDF…"), async () => {
  ...
  await withRouteProgress(async () => {
    try {
      ...
      if (parts[0] === "knowledge") {
        ...
        const openSheetConnection = parts[3] === "sheets";
        const deepCaseId = parts[3] && !openSheetConnection ? decodeURIComponent(parts[3]) : "";
        ...
        state.suiteSheetModalOpen = openSheetConnection;
        renderEditor();
      } else renderSuites();
      refreshIcons();
      window.scrollTo({ top: 0, behavior: "instant" });

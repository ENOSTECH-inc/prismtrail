...
const SECTION_STATUSES = ["needs_action", "no_issue"];
...
function normalizeSection(value = {}) {
  const normalizedActions = (Array.isArray(value.actions) ? value.actions : [])
    .map(normalizeAction)
    .filter((item) => item.proposal)
    .slice(0, 3);
  const summary = clipped(value.summary, 2_500);
  const status = SECTION_STATUSES.includes(value.status)
    ? value.status
...
export function failedCaseImprovementProposal(caseRun, message) {
  ...
  return {
    ...
    sections: Object.fromEntries(SECTION_KEYS.map((key) => [key, { status: "no_issue", summary: "", actions: [] }]))
  };
}

export { SECTION_KEYS as IMPROVEMENT_SECTION_KEYS, SECTION_STATUSES as IMPROVEMENT_SECTION_STATUSES };

...
function statusSlices(counts) {
  return [
    ...
    { label: "未評価", value: counts.skipped + counts.cancelled, color: GRAY }
  ];
}
...
function caseSpecSchemas() {
  return [
    ...
    rectSchema("metaDivider", { x: 102.5, y: 56, width: 0.25, height: 29, color: LINE }),
...
export function buildCaseSpecInputs({ suite, cases, agents = [] } = {}) {
  ...
  const inputs = (cases || []).flatMap((testCase, index) => {
    ...
    return criteriaPages.map((criteriaRows, criteriaPageIndex) => ({
        ...
        openLinkUrl: url,
        metaPrimary: `ケースID\n${testCase.id || "—"}\n\nデータエージェント\n${agent}`,
        metaSecondary: `実行条件\n${testCase.thinkingMode === "THINKING" ? "熟考" : "高速"}  ·  ${status}\n\n合格基準\n業務評価 ${passingGrade} 以上`,
        sectionMeta: "ケース情報",
...
function executiveFindings({ counts, passRate, report }) {
  const findings = [
    ...
    `・合格率は ${passRate}%、総合スコアは ${report.summary?.score ?? "—"}点（システム ${report.summary?.systemScore ?? "—"} / 業務 ${report.summary?.businessScore ?? "—"}）です。`
  ];
...
export function buildSuiteRunInputs({ report, caseIds = null, agents = [], runsById = {} } = {}) {
  ...
  const cover = {
    ...
    passRateNote: `評価済み ${evaluated || statusTotal || 0}件中 ${statusCounts.passed}件`,
    overallScoreLabel: "総合スコア",
    overallScoreValue: report.summary?.score == null ? "—" : `${report.summary.score}`,
    overallScoreNote: "総合品質評価",
    ...
    statusLegend: [
      ...
      `未評価    ${withCountPct(statusCounts.skipped + statusCounts.cancelled, statusTotal || 1)}`
    ].join("\n"),
  ...
  const indexPages = indexChunks.map((rows, pageIndex) =>
    populateIndexRowFields({
      ...
      indexHeadSystem: "システム",
      indexHeadBusiness: "業務",
      indexHeadScore: "総合",
  ...
  const casePages = selected.flatMap((testCase, index) => {
    ...
    const overview = {
      ...
      resultSummary: `総合 ${evaluation.score ?? "—"}点  ·  ${gradeText}  ·  SQL ${item.runSummary?.sqlCount ?? 0}件 / チャート ${item.runSummary?.chartCount ?? 0}件`,
      ...
      return populateBusinessItemFields({
        ...
        businessEmpty:
          business.status === "not_configured"
            ? "ビジネス要件は設定されていません。"
            : "判定項目または判定結果はありません。",
        sectionEvidence: lastBusinessPage ? "根拠プレビュー" : "続き",

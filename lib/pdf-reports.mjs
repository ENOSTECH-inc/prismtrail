...
function businessCriteriaRows(testCase = {}) {
  ...
  const sources = testCase.expectations?.accuracyValidation?.sources || [];
  for (const [index, source] of sources.entries()) {
    const type = { text: "TEXT", url: "URL", bigquery_sql: "BQ SQL" }[source.type] || source.type;
    const detail = source.description || source.content || "—";
    rows.push([`根拠${index + 1}`, clipText(`${type}: ${detail}`, 72)]);
  }
  if (!rows.length) return [["—", "ビジネス要件・精度検証: 未設定"]];
  return rows;
}
...
function businessReasonLines(business = {}) {
  ...
  const reasons = items
    ...
    .join("\n");
  const sourceLines = (business.accuracySources || []).map((source) =>
    `精度根拠 ${source.type || source.id}: ${source.status === "resolved" ? "取得済み" : `エラー ${source.error || "取得失敗"}`}`
  );
  return [reasons, ...sourceLines].filter(Boolean).join("\n") || "—";
}

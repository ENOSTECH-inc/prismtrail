...
function formatEvidenceBlock(evidence, runSummary = {}) {
  const answer = evidence?.answer ? clipNarrative(evidence.answer, PREVIEW_ANSWER_CLIP) : "（最終回答テキストなし）";
  const table = formatSampleTableText(evidence?.table);
  const chart = formatChartNote(evidence?.chart, runSummary);
  return [`【回答】${answer}`, clipText(table, 1_200), chart].join("\n");
}
...
function suiteRunImprovementSchemas() {
  const schemas = [
    ...pageHeaderSchemas({ compact: true, sectionAware: true, references: true }),
    ...sectionHeadingSchemas("sectionImprovement", 62, { titleName: "sectionImprovement" })
  ];
  const labels = ["① システムプロンプト", "② リファレンスクエリ", "③ 元mart", "④ その他"];
  for (let index = 0; index < 4; index += 1) {
    const y = 72 + index * 49.5;
    schemas.push(
      rectSchema(`improvementCard${index}`, {
        x: LEFT,
        y,
        width: CONTENT_WIDTH,
        height: 46,
      ...
      textSchema(`improvementLabel${index}`, {
        x: 18,
        y: y + 5,
        width: 42,
        height: 6,
      ...
      textSchema(`improvementBody${index}`, {
        x: 63,
        y: y + 4,
        width: 127,
        height: 38,
  ...
  schemas.push(
    textSchema("improvementDisclaimer", {
      x: LEFT,
      y: 271,
      width: CONTENT_WIDTH,
      height: 7,
...
/** PDF フィールド寸法に合わせた改善提案のページ分割（CJKは textWidth=2） */
const IMPROVEMENT_BODY_COLUMNS = 100;
const IMPROVEMENT_BODY_LINES = 11;
...
function improvementSectionStatusLabel(section = {}) {
  return improvementSectionStatus(section) === "needs_action" ? "要対応" : "問題なし";
}

function formatImprovementProposalSection(section = {}) {
  if (improvementSectionStatus(section) === "no_issue") return "";
  const lines = [];
  if (section.summary) lines.push(String(section.summary));
  for (const [index, action] of (section.actions || []).entries()) {
    lines.push(`${index + 1}. ${action.proposal || ""}`);
    if (action.rationale) lines.push(`根拠: ${action.rationale}`);
    if (action.expectedEffect) lines.push(`期待効果: ${action.expectedEffect}`);
  }
...
function suiteRunCaseEvidenceSchemas(input = {}) {
  const answerOnly = Boolean(input._evidenceAnswerOnly);
  ...
  const columns = headers.length
    ...
    : [];
  const answerCardHeight = answerOnly ? 198 : 72;
  const answerTextHeight = answerOnly ? 186 : 58;
  const schemas = [
    ...pageHeaderSchemas({ compact: true, sectionAware: true, showOpenLink: false }),
  ...
  schemas.push(
    ...sectionHeadingSchemas("sectionResponseData", 134, { titleName: "sectionResponseData" }),
    ...(columns.length
    ...
    ...sectionHeadingSchemas("sectionResponseChart", 178, { titleName: "sectionResponseChart" }),
    ...
    ...(input._hasChartSvg
      ? [svgSchema("responseChartSvg", { x: 22, y: 201, width: 166, height: 46 })]
      : [
...
const PDF_FAILED_STATUSES = new Set(["failed", "error"]);

/**
 * PDF 出力対象の caseId 一覧を解決する。
 * @param {object} report
 * @param {{ caseId?: string|null, scope?: "all"|"failed" }} [options]
 * @returns {string[]|null} null は「全件（スナップショット準拠）」
 */
export function resolveSuiteRunPdfCaseIds(report = {}, { caseId = null, scope = "all" } = {}) {
  const explicitCaseId = String(caseId || "").trim();
  if (explicitCaseId) return [explicitCaseId];

  if (isPartialSuiteRun(report)) {
    if (Array.isArray(report.selectedCaseIds) && report.selectedCaseIds.length) {
      return report.selectedCaseIds.map(String);
    }
    const first = (report.suiteSnapshot?.cases || report.caseRuns || [])
  ...
  const suiteCases = (report.suiteSnapshot?.cases || []).filter(
    (item) => String(item.status || "").toLowerCase() !== "draft"
  );
  let ids = (suiteCases.length ? suiteCases : report.caseRuns || [])
    .map((item) => item.id || item.caseId)
    .filter(Boolean)
    .map(String);

  if (String(scope || "all").toLowerCase() === "failed") {
    ids = ids.filter((id) => PDF_FAILED_STATUSES.has(completed.get(id)?.status));
  }
  return ids;
}
...
export function buildSuiteRunInputs({ report, caseIds = null, agents = [], runsById = {} } = {}) {
  ...
  const rollupReport = report.rollup?.type === "latest_per_case";
  ...
  const coverReferences = referenceLinkInputs({
    agent: agentRecord(agents, selected[0]?.agentId || report.suiteSnapshot?.defaultAgentId),
    suiteId: report.suiteId,
    reportId: rollupReport ? null : report.id
  });
  if (rollupReport) {
    coverReferences.referenceLabel2 = "集約方式";
    coverReferences.referenceValue2 = "ケースIDごとの最新保存結果";
    coverReferences._referenceLinks[2] = suiteEditorUrl(report.suiteId);
  }
  const cover = {
    ...SECTION_ICON_INPUTS,
    ...coverReferences,
    ...
    docType: rollupReport ? "ケース別最新結果サマリー" : "評価実行サマリー",
    title: clipText(report.reportTitle || report.suiteName || "評価実行レポート", 52),
    subtitle: rollupReport
      ? `ケースIDで最新結果を集約  /  スイート ${report.suiteId || "—"}  /  ${generatedAt}`
      : `実行 ${report.id}  /  スイート ${report.suiteId || "—"}  /  ${generatedAt}`,
    openLink: rollupReport
      ? "▶ PrismTrailでテストスイートを開く"
      : "▶ PrismTrailでこの評価レポートを開く",
    openLinkUrl: "",
  ...
  const casePages = selected.flatMap((testCase, index) => {
    ...
    const overview = {
      ...
      ...referenceLinkInputs({
        ...
        suiteId: report.suiteId,
        reportId: item.rollupSource?.suiteRunId || report.id
      }),
    ...
    const businessItems = (Array.isArray(business.itemResults) ? business.itemResults : []).map(
      (entry) => {
        ...
        const reason = clipText(rawReason, 400);
        return {
          ...display,
          criterion,
          reason,
    ...
    const answerSource = clipNarrative(
      evidence?.answer || "最終回答テキストはありません。",
      PREVIEW_ANSWER_CLIP
    );
    // 1ページ目は表・チャート併記（回答欄 ~58mm）、続きは回答全文ページ
    const answerFirstPages = paginateTraceText(answerSource, { columns: 100, lines: 14 });
    const answerOverflow = answerFirstPages.slice(1).join("\n");
    const answerRestPages = answerOverflow
      ? paginateTraceText(answerOverflow, { columns: 100, lines: 42 })
      : [];
    const evidencePages = evidence
      ? [
          populateGridFields(
            {
              ...
              sectionResponseAnswer: `回答${
                answerFirstPages.length + answerRestPages.length > 1
                  ? `  1 / ${answerFirstPages.length + answerRestPages.length}`
                  : ""
              }`,
              responseAnswer: answerFirstPages[0] || "最終回答テキストはありません。",
          ...
          ...answerRestPages.map((pageText, restIndex) => ({
            ...shared,
            _pageKind: "case-evidence",
            _evidenceAnswerOnly: true,
            _chartPreview: null,
            _evidenceTableHeaders: [],
            _evidenceTableRows: [],
            _hasChartSvg: false,
            sectionStep: "セクション 3 / 3",
            sectionName: "回答・データ・チャート",
            ...
            openLink: item.runId
              ? "▶ PrismTrailでこのケースの実行詳細を開く"
              : "▶ PrismTrailで評価レポートを開く",
            sectionResponseAnswer: `回答  ${restIndex + 2} / ${answerRestPages.length + 1}`,
            responseAnswer: pageText,
            footer: footerBase,
            pageLabel: ""
          }))
        ]
    ...
    const improvementPages = proposal?.status === "succeeded"
      ? (() => {
          const keys = ["systemPrompt", "referenceQuery", "sourceMart", "other"];
          const baseLabels = ["① システムプロンプト", "② リファレンスクエリ", "③ 元mart", "④ その他"];
          const labels = keys.map((key, index) =>
            `${baseLabels[index]}  ［${improvementSectionStatusLabel(proposal.sections?.[key])}］`
          );
          const pagesBySection = keys.map((key) =>
            paginateTraceText(formatImprovementProposalSection(proposal.sections?.[key]), {
              columns: IMPROVEMENT_BODY_COLUMNS,
              lines: IMPROVEMENT_BODY_LINES
            })
          );
          const pageCount = Math.max(1, ...pagesBySection.map((pages) => pages.length));
          return Array.from({ length: pageCount }, (_, proposalPageIndex) => ({
            ...shared,
            ...referenceLinkInputs({
              agent: agentRecord(agents, testCase.agentId || report.suiteSnapshot?.defaultAgentId),
              suiteId: report.suiteId,
            ...
            _pageKind: "case-improvement",
            sectionStep: "補足",
            sectionName: "改善提案",
            docType: "",
            openLink: "▶ PrismTrailで改善提案の全文を確認する",
            sectionImprovement: `Gemini改善提案${pageCount > 1 ? `  ${proposalPageIndex + 1} / ${pageCount}` : ""}`,
            improvementLabel0: labels[0],
            improvementLabel1: labels[1],
            improvementLabel2: labels[2],
            improvementLabel3: labels[3],
            improvementBody0: improvementSectionStatus(proposal.sections?.[keys[0]]) === "no_issue" ? "" : pagesBySection[0][proposalPageIndex] ?? "（前ページに掲載）",
            improvementBody1: improvementSectionStatus(proposal.sections?.[keys[1]]) === "no_issue" ? "" : pagesBySection[1][proposalPageIndex] ?? "（前ページに掲載）",
            improvementBody2: improvementSectionStatus(proposal.sections?.[keys[2]]) === "no_issue" ? "" : pagesBySection[2][proposalPageIndex] ?? "（前ページに掲載）",
            improvementBody3: improvementSectionStatus(proposal.sections?.[keys[3]]) === "no_issue" ? "" : pagesBySection[3][proposalPageIndex] ?? "（前ページに掲載）",
            improvementDisclaimer: `Geminiによる非権威的な提案です。設定を自動変更せず、評価・合否にも影響しません。 / ${proposal.model || "Gemini"} / ${formatJst(proposal.generatedAt)}`,
            footer: footerBase,
            pageLabel: ""
          }));
        })()
    ...
    return [overview, ...tracePages, ...detailPages, ...evidencePages, ...improvementPages];
  });
...
function reportHeaderBrand(input = {}) {
  ...
  if (["case-overview", "case-trace", "case-detail", "case-evidence", "case-improvement"].includes(input._pageKind)) {
...
function schemasForInput(input) {
  ...
  if (input._pageKind === "case-improvement") return suiteRunImprovementSchemas(input);
...
export function pdfFilename(kind, id) {
  ...
  if (kind === "latest-run") return `prismtrail-suite-${safe}-latest-run.pdf`;
  if (kind === "latest-case-results") return `prismtrail-suite-${safe}-latest-case-results.pdf`;

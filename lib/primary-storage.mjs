...
export class GcsStorageBackend {
  ...
  async request(url, options = {}, { retryAuth = true } = {}) {
    ...
    if (!response.ok) {
      ...
      if (response.status === 429) error.code = "ERATELIMIT";
      throw error;
    }
  ...
  async save(namespace, value, { ifAbsent = false, maxRetries = 5 } = {}) {
    ...
    for (let attempt = 1; attempt <= attempts; attempt += 1) {
      ...
      try {
        ...
        lastError = error;
        const retriableConflict = !ifAbsent && error?.code === "EEXIST" && error?.status === 412;
        const retriableRateLimit = error?.status === 429;
        if (!retriableConflict && !retriableRateLimit) throw error;
        ...
        if (retriableConflict) {
          ...
          try {
            ...
            if (metadata?.generation != null) {
              this.generations.set(cacheKey, String(metadata.generation));
            }
          } catch (metaError) {
            if (metaError?.code !== "ENOENT") throw metaError;
            this.generations.set(cacheKey, "0");
          }
          console.warn(
            `[storage] GCS generation conflict on ${cacheKey}; retrying (${attempt}/${attempts})`
          );
        } else {
          const delayMs = Math.min(1000 * 2 ** (attempt - 1), 8_000);
          console.warn(
            `[storage] GCS rate limit on ${cacheKey}; retrying in ${delayMs}ms (${attempt}/${attempts})`
          );

...
/** UI/PDF の本文向け。短文を刈りすぎないよう余裕を持たせる */
export const NARRATIVE_CLIP = 8_000;
export const PREVIEW_ANSWER_CLIP = 8_000;

/** 改行を保ったまま刈る（回答・改善提案などの長文向け） */
export function clipNarrative(value, max = NARRATIVE_CLIP) {
...
export function buildRunEvidencePreview(run = {}) {
  if (!run) return null;
  const answer = clipNarrative(extractFinalResponseText(run), PREVIEW_ANSWER_CLIP);

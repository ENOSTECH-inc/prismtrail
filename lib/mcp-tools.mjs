...
export function createPrismTrailMcpTools(operations) {
  return [
    ...
    {
      name: "list_sheet_connections", scope: "sheets:read", description: "List Data-Agent-scoped Google Sheets connections, optionally filtered by registered agent id.",
      inputSchema: objectSchema({ agentId: stringId }), handler: (args) => operations.listSheetConnections(args)
    },
    {
      name: "connect_google_sheet", scope: "sheets:write", description: "Register one named Google Sheet, link it to exactly one Data Agent, and bootstrap only that agent's managed data.",
      inputSchema: objectSchema({ spreadsheetUrl: { type: "string" }, sheetName: { type: "string", minLength: 1, maxLength: 120 }, agentId: stringId, suiteId: stringId, forceOperational: { type: "boolean" } }, ["spreadsheetUrl", "sheetName", "agentId"]), handler: (args) => operations.connectSheet(args)
    },
    {
      name: "check_sheet_connection", scope: "sheets:write", description: "Verify and refresh a Data-Agent-scoped Google Sheets connection.",
      inputSchema: objectSchema({ connectionId: stringId, suiteId: stringId }, ["connectionId"]), handler: (args) => operations.checkSheet(args)
    },
    {
      name: "export_suite_to_sheet", scope: "sheets:write", description: "Export a suite to its matching Data Agent's sheet and refresh only that agent's catalogs.",
    ...
    {
      name: "import_suite_from_sheet", scope: "sheets:write", description: "Import a managed suite only when every case matches the connection's Data Agent.",
    ...
    {
      name: "export_report_to_sheet", scope: "sheets:write", description: "Export an evaluation report only to the sheet linked to its Data Agent.",

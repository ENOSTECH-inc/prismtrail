...
export function createPrismTrailMcpTools(operations) {
  return [
    ...
    {
      name: "create_test_case", scope: "suites:write", description: "Add a new test case. businessRequirements.criteriaItems defines the acceptance conditions judged against the Data Agent response. relatedUrls is provenance only.",
    ...
    {
      name: "update_test_case", scope: "suites:write", description: "Update one test case without deleting others. businessRequirements.criteriaItems defines the acceptance conditions; relatedUrls remains provenance only.",
    ...
    {
      name: "list_sheet_connections", scope: "sheets:read", description: "List test-suite-scoped Google Sheets connections, optionally filtered by suite id.",
      inputSchema: objectSchema({ suiteId: stringId, agentId: stringId }), handler: (args) => operations.listSheetConnections(args)
    },
    {
      name: "connect_google_sheet", scope: "sheets:write", description: "Register one named Google Sheet, link it one-to-one with a test suite, and bootstrap that suite's managed data.",
      inputSchema: objectSchema({ spreadsheetUrl: { type: "string" }, sheetName: { type: "string", minLength: 1, maxLength: 120 }, suiteId: stringId, forceOperational: { type: "boolean" } }, ["spreadsheetUrl", "sheetName", "suiteId"]), handler: (args) => operations.connectSheet(args)
    },
    {
      name: "check_sheet_connection", scope: "sheets:write", description: "Verify and refresh a test-suite-scoped Google Sheets connection.",
      inputSchema: objectSchema({ connectionId: stringId, suiteId: stringId }, ["connectionId"]), handler: (args) => operations.checkSheet(args)
    },
    {
      name: "export_suite_to_sheet", scope: "sheets:write", description: "Export a suite to its one-to-one Sheet connection and refresh its Suite-scoped catalogs.",
      inputSchema: objectSchema({ connectionId: stringId, suiteId: stringId }, ["suiteId"]), handler: (args) => operations.exportSuiteToSheet(args)
    },
    {
      name: "import_suite_from_sheet", scope: "sheets:write", description: "Update the one test suite bound to a managed Sheet; mismatched embedded Suite IDs are rejected.",
      inputSchema: objectSchema({ connectionId: stringId, suiteId: stringId }, ["suiteId"]), handler: (args, context) => operations.importSuiteFromSheet(args, context)
    },
    {
      name: "export_report_to_sheet", scope: "sheets:write", description: "Export an evaluation report to the Sheet linked to its originating test suite.",
      inputSchema: objectSchema({ connectionId: stringId, reportId: stringId }, ["reportId"]), handler: (args) => operations.exportReportToSheet(args)
    },
    {

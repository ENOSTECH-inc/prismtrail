import { GOOGLE_ADC_SCOPES, GOOGLE_SHEETS_SCOPES, getAccessToken } from "./google-cloud.mjs";
...
export const GOOGLE_SHEETS_ADC_COMMAND =
  `gcloud auth application-default login --scopes=${GOOGLE_SHEETS_SCOPES.join(",")}`;
...
export async function diagnoseGoogleAuth({
  ...
  try {
    ...
    fullScopeError = error;
    const fallback = sheetsTokenProvider || (tokenProvider === getAccessToken
      ? () => getAccessToken({ scopes: GOOGLE_SHEETS_SCOPES })
      : null);
    if (fallback) {
      try {
        tokenResult = await fallback();
      } catch {
        // Report the original full-scope failure below.
      }
    }
    if (!tokenResult) {
      return {
        ...
        message: `Application Default Credentialsを利用できません: ${fullScopeError.message}`
      };
    }
  }
  ...
  try {
    ...
    return {
      ...
      missingScopes,
      command: GOOGLE_ADC_IMPERSONATION_COMMAND,
      setupMode: features.find((feature) => feature.id === "sheets")?.granted && !features.find((feature) => feature.id === "cloud")?.granted
        ? "sheets-only"
        : "full",
      message: missingScopes.length
        ? features.find((feature) => feature.id === "sheets")?.granted && !features.find((feature) => feature.id === "cloud")?.granted
          ? "ユーザーADCでSheetsを利用できます。GCSやData Agent操作にはCloud scopeが必要です。"
          : "ADCは利用できますが、必要なOAuth scopeが不足しています。"

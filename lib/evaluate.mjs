...
export function composeEvaluation(systemEvaluation, judgeResult, businessRequirements = {}) {
  ...
  const accuracyValidation = businessRequirements.accuracyValidation || {};
  const hasSeparatedAccuracyValidation = Object.hasOwn(businessRequirements, "accuracyValidation");
  const enabled = businessRequirements.enabled !== false && criteriaItems.length > 0 && (
    !hasSeparatedAccuracyValidation ||
    (accuracyValidation.enabled !== false && Array.isArray(accuracyValidation.sources) && accuracyValidation.sources.length > 0)
  );
  if (!enabled) {
    return {
      ...systemEvaluation,
  ...
  if (judgeResult?.evaluationError) {
    const business = {
      ...
      accuracySources: judgeResult.resolvedAccuracySources || []
    };
    return {
      ...systemEvaluation,
      schemaVersion: 3,

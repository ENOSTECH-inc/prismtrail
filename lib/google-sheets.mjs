...
const SUPPORTED_SHEET_SCHEMA_VERSIONS = ["1", "2", "3", "4", "5", "6", SHEET_SCHEMA_VERSION];
...
export function reportToRows(report) {
  ...
  return [
    ...
    ["", "◆ ビジネス要件はH〜O、レスポンス受領はT〜U、Gemini改善提案はV〜ABに表示されます"],
    ...
    return {
      ...
      ...(sample.score < 100 ? {
        improvementProposal: {
          status: "succeeded",
          sections: {
            systemPrompt: { summary: "期間解釈を明示します", actions: [{ proposal: "対象期間の優先規則を追記する", rationale: "月の解釈差を減らす", expectedEffect: "期間誤りを抑える" }] },
            referenceQuery: { summary: "代表SQLを追加します", actions: [] },
            sourceMart: { summary: "必要列を確認します", actions: [] },
            other: { summary: "再実行で確認します", actions: [] }
          },
...
export function writeReportSheet(spreadsheetId, report) {
  return writeFixedSheet(spreadsheetId, REPORT_SHEET, reportToRows(report), 15, {
    columnWidths: [150, 230, 105, 100, 115, 110, 320, 90, 75, 105, 130, 320, 420, 300, 170, 125, 145, 175, 250, 125, 115, 120, 360, 360, 360, 360, 170, 190],
    ...
    customRequests: ({ sheetId, headerRowIndex, dataStartRowIndex, dataEndRowIndex }) => [
      ...
      repeatFormat(
        gridRange(sheetId, dataStartRowIndex, dataEndRowIndex, 22, 26),

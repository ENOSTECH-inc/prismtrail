...
export const SHEET_SCHEMA_VERSION = "8";
const SUPPORTED_SHEET_SCHEMA_VERSIONS = ["1", "2", "3", "4", "5", "6", "7", SHEET_SCHEMA_VERSION];
...
export const REPORT_HEADERS = [
  ...
  "http_status",
  "improvement_status",
  "improvement_system_prompt_status",
  "improvement_system_prompt",
  "improvement_reference_query_status",
  "improvement_reference_query",
  "improvement_source_mart_status",
  "improvement_source_mart",
  "improvement_other_status",
  "improvement_other",
  "improvement_model",
...
function formatImprovementSectionCell(section = {}) {
...
function improvementSectionStatus(section = null) {
  if (!section) return "";
...
function improvementSectionStatusLabel(proposal, section) {
  if (proposal?.status !== "succeeded") return "";
  return improvementSectionStatus(section) === "needs_action" ? "要対応" : "問題なし";
}

function improvementStatusLabel(proposal) {
  if (!proposal) return "不明";
  return {
    succeeded: "生成済み",
    failed: "生成失敗",
    generating: "生成中",
    pending: "生成待ち",
    not_applicable: "対象外"
  }[proposal.status] || proposal.status || "不明";
}

export function reportToRows(report) {
  const latestPerCase = report.rollup?.type === "latest_per_case";
  ...
  const responseReceiptLabel = (status) => ({
    received: "受領済み",
    not_received: "未受領",
    not_run: "未実行",
    pending: "待機中"
  ...
  return [
    [latestPerCase ? "PrismTrail | ケース別最新結果レポート" : "PrismTrail | 評価レポート"],
    ["スキーマ", SHEET_SCHEMA_VERSION],
    ["出力日時", new Date().toISOString()],
    [latestPerCase ? "ロールアップレポートID" : "Suite Run ID", report.id],
    ...
    [
      latestPerCase ? "集約方式" : "",
      `${latestPerCase ? `ケースIDごとの最新保存結果（元Suite Run ${report.rollup?.sourceSuiteRunIds?.length || 0}件）\n` : ""}◆ ビジネス要件はH〜O、レスポンス受領はT〜U、Gemini改善提案はV〜AFに表示されます`
    ],
    ...
    ...(report.caseRuns || []).map((item) => [
      ...
      item.runId || "",
      item.error || "",
      responseReceiptLabel(item.responseReceipt?.status),
      item.responseReceipt?.httpStatus ?? "",
      improvementStatusLabel(item.improvementProposal),
      improvementSectionStatusLabel(item.improvementProposal, item.improvementProposal?.sections?.systemPrompt),
      formatImprovementSectionCell(item.improvementProposal?.sections?.systemPrompt),
      improvementSectionStatusLabel(item.improvementProposal, item.improvementProposal?.sections?.referenceQuery),
      formatImprovementSectionCell(item.improvementProposal?.sections?.referenceQuery),
      improvementSectionStatusLabel(item.improvementProposal, item.improvementProposal?.sections?.sourceMart),
      formatImprovementSectionCell(item.improvementProposal?.sections?.sourceMart),
      improvementSectionStatusLabel(item.improvementProposal, item.improvementProposal?.sections?.other),
      formatImprovementSectionCell(item.improvementProposal?.sections?.other) || item.improvementProposal?.message || "",
      item.improvementProposal?.model || "",
      item.improvementProposal?.generatedAt || ""
    ])
  ];
}
    ...
    return {
      ...
      ...(sample.score < 100 ? {
        improvementProposal: {
          status: "succeeded",
          sections: {
            systemPrompt: { status: "needs_action", summary: "期間解釈を明示します", actions: [{ proposal: "対象期間の優先規則を追記する", rationale: "月の解釈差を減らす", expectedEffect: "期間誤りを抑える" }] },
            referenceQuery: { status: "needs_action", summary: "代表SQLを追加します", actions: [] },
            sourceMart: { status: "no_issue", summary: "", actions: [] },
            other: { status: "needs_action", summary: "再実行で確認します", actions: [] }
          },
...
export function sheetColumnLabel(columnCount) {
...
async function writeFixedSheet(
  ...
  const existingColumnCount = Number(sheet.properties?.gridProperties?.columnCount || 0);
  if (existingColumnCount < lastColumn) {
    await googleRequest(`spreadsheets/${encodeURIComponent(spreadsheetId)}:batchUpdate`, {
      method: "POST",
      body: {
        requests: [{
          updateSheetProperties: {
            properties: { sheetId: sheet.properties.sheetId, gridProperties: { columnCount: lastColumn } },
  ...
  const cleanupColumnCount = Math.max(existingColumnCount, lastColumn);
  const range = `${quoteSheet(title)}!A:${sheetColumnLabel(cleanupColumnCount)}`;
  await googleRequest(
  ...
  await googleRequest(`spreadsheets/${encodeURIComponent(spreadsheetId)}:batchUpdate`, {
    ...
    body: {
      requests: [
        ...
        {
          unmergeCells: {
            range: gridRange(sheetId, 0, 1000, 0, cleanupColumnCount)
          }
        },
        {
          setDataValidation: {
...
export function writeReportSheet(spreadsheetId, report) {
  return writeFixedSheet(spreadsheetId, REPORT_SHEET, reportToRows(report), 15, {
    columnWidths: [150, 230, 105, 100, 115, 110, 320, 90, 75, 105, 130, 320, 420, 300, 170, 125, 145, 175, 250, 125, 115, 120, 100, 360, 100, 360, 100, 360, 100, 360, 170, 190],
    ...
    customRequests: ({ sheetId, headerRowIndex, dataStartRowIndex, dataEndRowIndex }) => [
        ...
        setDataValidation: {
          ...
          rule: {
            condition: {
              type: "ONE_OF_LIST",
              values: [
                { userEnteredValue: "要対応" },
                { userEnteredValue: "問題なし" }
              ]
            },
            strict: true,
            showCustomUi: true
      ...
      {
        setDataValidation: {
          range: gridRange(sheetId, dataStartRowIndex, Math.max(dataEndRowIndex, dataStartRowIndex + MAX_SUITE_CASES), 21, 22),
          rule: {
            condition: {
              type: "ONE_OF_LIST",
              values: [
                { userEnteredValue: "生成済み" },
                { userEnteredValue: "生成失敗" },
                { userEnteredValue: "生成中" },
                { userEnteredValue: "生成待ち" },
                { userEnteredValue: "対象外" },
                { userEnteredValue: "不明" }
              ]
            },
            strict: true,
      ...
      {
        setDataValidation: {
          range: gridRange(sheetId, dataStartRowIndex, Math.max(dataEndRowIndex, dataStartRowIndex + MAX_SUITE_CASES), 19, 20),
          rule: {
            condition: {
              type: "ONE_OF_LIST",
              values: [
                { userEnteredValue: "受領済み" },
                { userEnteredValue: "未受領" },
                { userEnteredValue: "未実行" },
                { userEnteredValue: "待機中" },
                { userEnteredValue: "不明" }
              ]
            },
            strict: true,
      ...
      repeatFormat(
        gridRange(sheetId, dataStartRowIndex, dataEndRowIndex, 20, 21),
        { numberFormat: { type: "NUMBER", pattern: "0" }, horizontalAlignment: "RIGHT" },
        "userEnteredFormat.numberFormat,userEnteredFormat.horizontalAlignment"
      ),
      ...[
        ["受領済み", COLORS.greenSoft, COLORS.green],
        ["未受領", COLORS.redSoft, COLORS.red],
        ["未実行", COLORS.panel, COLORS.muted]
      ].map(([value, backgroundColor, foregroundColor]) => ({
        addConditionalFormatRule: {
          index: 0,
          rule: {
            ranges: [gridRange(sheetId, dataStartRowIndex, dataEndRowIndex, 19, 20)],
            booleanRule: {
      ...
      ...[
        ["生成済み", COLORS.greenSoft, COLORS.green],
        ["生成失敗", COLORS.redSoft, COLORS.red],
        ["生成中", COLORS.amberSoft, COLORS.amber],
        ["対象外", COLORS.panel, COLORS.muted],
        ["不明", COLORS.panel, COLORS.muted]
      ].map(([value, backgroundColor, foregroundColor]) => ({
        addConditionalFormatRule: {
          index: 0,
          rule: {
            ranges: [gridRange(sheetId, dataStartRowIndex, dataEndRowIndex, 21, 22)],
            booleanRule: {
              condition: { type: "TEXT_EQ", values: [{ userEnteredValue: value }] },
              format: { backgroundColor, textFormat: { foregroundColor, bold: true } }
            }
      ...
      ...[23, 25, 27, 29].map((columnIndex) => repeatFormat(
        gridRange(sheetId, dataStartRowIndex, dataEndRowIndex, columnIndex, columnIndex + 1),
        { backgroundColor: COLORS.blueSoft, wrapStrategy: "WRAP", verticalAlignment: "TOP" }
      )),
      ...[
        ["要対応", COLORS.amberSoft, COLORS.amber],
        ["問題なし", COLORS.greenSoft, COLORS.green]
      ].flatMap(([value, backgroundColor, foregroundColor]) =>
        [22, 24, 26, 28].map((columnIndex) => ({
          addConditionalFormatRule: {
            index: 0,
            rule: {
              ranges: [gridRange(sheetId, dataStartRowIndex, dataEndRowIndex, columnIndex, columnIndex + 1)],
              booleanRule: {
                condition: { type: "TEXT_EQ", values: [{ userEnteredValue: value }] },
                format: { backgroundColor, textFormat: { foregroundColor, bold: true } }
              }

  ...
  return [
    {
      ...
      expectations: {
        ...
        businessRequirements: {
          enabled: true,
          accuracyCriteria: "6月のMAUが数値で回答され、対象期間が6月であることが明確であること",
          passingGrade: "B"
        }
      }
    },
    {
      ...
      expectations: {
        ...
        businessRequirements: {
          enabled: true,
          accuracyCriteria: "チャネル別のユーザー数が表またはチャートで示されていること",
          passingGrade: "B"
        }
      }
    },
    {
      ...
      expectations: {
        ...
        businessRequirements: {
          enabled: true,
          accuracyCriteria: "5月・6月のMAUと成長率（または差分）が回答に含まれていること",
          passingGrade: "B"
        }
      }
    }
    ...
    return {
      ...
      evaluation: {
        ...
        business: {
          ...
          expectedCriteria: testCase.expectations?.businessRequirements?.accuracyCriteria || "",
          evidence: [
            {
              quote: "サンプル根拠テキスト",
...
export function writeSuiteSheet(spreadsheetId, suite, { agents = [] } = {}) {
  ...
  return writeFixedSheet(spreadsheetId, SUITE_SHEET, suiteToRows(suite, { agents }), 11, {
    columnWidths: [150, 220, 520, 180, 110, 100, 95, 105, 125, 145, 200, 420, 210],
    tabColor: COLORS.blue,
    customRequests: ({ sheetId, headerRowIndex, dataStartRowIndex, dataEndRowIndex, lastColumn }) => [
      ...
      repeatFormat(
        gridRange(sheetId, headerRowIndex, headerRowIndex + 1, 11, 12),
        {
      ...
      ...(agentIds.length
        ? [
            {
              setDataValidation: {
                range: gridRange(sheetId, dataStartRowIndex, dataStartRowIndex + 50, 3, 4),
                rule: {
                  condition: {
                    type: "ONE_OF_LIST",
      ...
      {
        setDataValidation: {
          range: gridRange(sheetId, dataStartRowIndex, dataStartRowIndex + 50, 5, 6),
          rule: {
            condition: {
              type: "ONE_OF_LIST",
      ...
      repeatFormat(
        gridRange(sheetId, dataStartRowIndex, dataStartRowIndex + 50, 9, 10),
        { numberFormat: { type: "NUMBER", pattern: "#,##0" }, horizontalAlignment: "RIGHT" },
        "userEnteredFormat.numberFormat,userEnteredFormat.horizontalAlignment"
      ),
      repeatFormat(
        gridRange(sheetId, dataStartRowIndex, dataStartRowIndex + 50, 11, 12),
        {
      ...
      },
      {
        addConditionalFormatRule: {
          index: 0,
          rule: {
            ranges: [gridRange(sheetId, dataStartRowIndex, dataStartRowIndex + 50, 5, 6)],
            booleanRule: {
...
export function writeReportSheet(spreadsheetId, report) {
  return writeFixedSheet(spreadsheetId, REPORT_SHEET, reportToRows(report), 15, {
    ...
    customRequests: ({ sheetId, headerRowIndex, dataStartRowIndex, dataEndRowIndex }) => [
      ...
      ...[2, 4].map((columnIndex) => ({
        setDataValidation: {
          range: gridRange(sheetId, dataStartRowIndex, Math.max(dataEndRowIndex, dataStartRowIndex + 50), columnIndex, columnIndex + 1),
          rule: {
            condition: {
              type: "ONE_OF_LIST",
              values: [
      ...
      {
        setDataValidation: {
          range: gridRange(sheetId, dataStartRowIndex, Math.max(dataEndRowIndex, dataStartRowIndex + 50), 7, 8),
          rule: {
            condition: {
              type: "ONE_OF_LIST",
              values: [
      ...
      {
        setDataValidation: {
          range: gridRange(sheetId, dataStartRowIndex, Math.max(dataEndRowIndex, dataStartRowIndex + 50), 10, 11),
          rule: {
            condition: {
              type: "ONE_OF_LIST",
              values: [

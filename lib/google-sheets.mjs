import { getAccessToken, GOOGLE_SHEETS_SCOPES } from "./google-cloud.mjs";
import { formatCriteriaItems, parseCriteriaItems } from "./evaluate.mjs";
import { normalizeRelatedUrls } from "./related-urls.mjs";
import { parseAccuracySourcesCell, serializeAccuracySources } from "./accuracy-sources.mjs";
...
const SUPPORTED_SHEET_SCHEMA_VERSIONS = ["1", "2", "3", SHEET_SCHEMA_VERSION];
...
const LEGACY_SUITE_HEADERS = SUITE_HEADERS.filter(
  (header) => header !== "accuracy_requirement" && header !== "status"
);
...
const SUITE_HEADER_ALIASES = new Map([
  ...
  ["ビジネス要件の検証内容（自然言語）", "accuracy_requirement"],
  ["ビジネス要件チェック（;区切り）", "accuracy_requirement"],
  ["ビジネス要件チェック", "accuracy_requirement"],
  ["ビジネス受入条件（;区切り）", "accuracy_requirement"],
  ["ビジネス受入条件", "accuracy_requirement"],
...
function expectedSuiteHeaders(headers = []) {
  ...
  if (!headers.includes("accuracy_requirement")) {
    expected = expected.filter((header) => header !== "accuracy_requirement");
  }
  if (!headers.includes("accuracy_sources_json")) {
    expected = expected.filter((header) => header !== "accuracy_sources_json");
  }
...
export function suiteToRows(suite, { agents = [] } = {}) {
  ...
  return [
    ...
    ...(suite.cases || []).map((testCase) => [
      ...
      serializeAccuracySources(testCase.expectations?.accuracyValidation?.sources || []),
...
export function rowsToSuiteInput(rows) {
  ...
  const cases = rows
    ...
    .map((row, index) => {
      ...
      const accuracyCell = stringCell(cell(row, "accuracy_requirement"));
      if (accuracyCell.length > 5000) {
        throw new Error(`${rowNumber}行目: ビジネス要件チェックは5,000文字以内で指定してください。`);
      }
      const criteriaItems = parseCriteriaItems(accuracyCell);
      const accuracyCriteria = formatCriteriaItems(criteriaItems);
      let accuracySources;
      try {
        accuracySources = parseAccuracySourcesCell(cell(row, "accuracy_sources_json"));
      } catch (error) {
        throw new Error(`${rowNumber}行目: ${error.message}`);
      }
      if (!columnIndex.has("accuracy_sources_json") && criteriaItems.length) {
        accuracySources = [{
          id: "legacy_text",
          type: "text",
          description: "旧形式から移行した正解根拠",
          content: accuracyCriteria
        }];
      }
      ...
      return {
        ...
        expectations: {
          ...
          accuracyValidation: {
            enabled: criteriaItems.length > 0 && accuracySources.length > 0,
...
export function reportToRows(report) {
  ...
  return [
    ...
    ["", "◆ ビジネス要件と精度検証の結果は、紫色の列（H〜P）に表示されます"],
    ...
    ...(report.caseRuns || []).map((item) => [
      ...
      (item.evaluation?.business?.accuracySources || [])
        .map((source) => `${source.status === "resolved" ? "✓" : "×"} ${source.type || source.id}${source.description ? `: ${source.description}` : ""}${source.error ? ` — ${source.error}` : ""}`)
        .join("\n"),
...
async function googleRequest(path, { method = "GET", body, timeoutMs = 30_000 } = {}) {
  const { token, source: authSource } = await getAccessToken({ scopes: GOOGLE_SHEETS_SCOPES });
  ...
  return [
    {
      ...
      expectations: {
        ...
        businessRequirements: {
          ...
          accuracyCriteria: "6月のMAUが数値で回答されている; 対象期間が6月であることが明確",
          passingGrade: "B"
        }
      }
    },
    {
      ...
      expectations: {
        ...
        businessRequirements: {
          ...
          accuracyCriteria: "チャネル別のユーザー数が示されている; 表またはチャートがある",
          passingGrade: "B"
        }
      }
    },
    {
      ...
      expectations: {
        ...
        businessRequirements: {
          ...
          accuracyCriteria: "5月と6月のMAUが回答に含まれる; 成長率または差分が示されている",
          passingGrade: "B"
        }
      }
    }
...
export function emptyReportTemplate(suite = {}) {
  return {
    ...
    summary: {
      ...
      systemScore: 0,
      businessScore: "未設定",
      accuracyGrades: { A: 0, B: 0, C: 0, D: 0 }
    },
    caseRuns: []
  };
}
...
export function writeSuiteSheet(spreadsheetId, suite, { agents = [] } = {}) {
  ...
  return writeFixedSheet(spreadsheetId, SUITE_SHEET, suiteToRows(suite, { agents }), 11, {
    columnWidths: [150, 220, 520, 180, 110, 100, 95, 105, 125, 145, 180, 200, 420, 180, 360],
    tabColor: COLORS.blue,
    customRequests: ({ sheetId, headerRowIndex, dataStartRowIndex, dataEndRowIndex, lastColumn }) => [
      ...
      repeatFormat(
        gridRange(sheetId, dataStartRowIndex, dataStartRowIndex + MAX_SUITE_CASES, 14, 15),
...
export function writeReportSheet(spreadsheetId, report) {
  return writeFixedSheet(spreadsheetId, REPORT_SHEET, reportToRows(report), 15, {
    columnWidths: [150, 230, 105, 100, 115, 110, 320, 90, 75, 105, 130, 320, 420, 260, 300, 170, 125, 145, 175, 250],
    ...
    customRequests: ({ sheetId, headerRowIndex, dataStartRowIndex, dataEndRowIndex }) => [
      ...
      repeatFormat(
        gridRange(sheetId, headerRowIndex, headerRowIndex + 1, 7, 16),
        {
          backgroundColor: COLORS.purple,
          textFormat: {
            foregroundColor: COLORS.white,
      ...
      repeatFormat(
        gridRange(sheetId, dataStartRowIndex, dataEndRowIndex, 7, 16),
        {
          backgroundColor: COLORS.purpleSoft,
          textFormat: { foregroundColor: COLORS.ink },
          wrapStrategy: "WRAP"
      ...
      repeatFormat(
        gridRange(sheetId, dataStartRowIndex, dataEndRowIndex, 17, 18),
        {
          numberFormat: {
            type: "NUMBER",
